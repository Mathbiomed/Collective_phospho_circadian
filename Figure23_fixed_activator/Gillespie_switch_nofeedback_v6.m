function [Y_entry, Z_entry,X_list]=Gillespie_switch_nofeedback_v6(lambda_p, lambda_n, lambda_d, delay_mean, delay_cv,initial_particle_num, A,repeat_time)
...
degradation_list=NaN(max_n, 2, repeat_time);
Y_entry=NaN(max_n,repeat_time);
Z_entry=NaN(max_n,repeat_time);
...
%     temp_degradation_list=NaN(max_n,2);
    temp_Y_entry_list=NaN(max_n,1);
    temp_Z_entry_list=NaN(max_n,1);

    x(1)=initial_particle_num;
    stack_time = gamrnd(delay_alpha, delay_beta,initial_particle_num,1);
    stack_time_0 = stack_time;
    ...
    while (t<max_t)&&(iter<max_iter)
        ...
        prop_vector(1) = 0; % No protein production other than initiation
        prop_vector(2) = phos(x(2:3),switch_scale,1,vol); % protein phosphorylation at peri
        ...
        prop_vector(7)=lambda_d*x(3); % degradation of protein in perinucleus(phos)
        prop_vector(8)=lambda_d*x(4); % degradation of protein in nucleus
        ...
        if (t>=time_step*time_index)
            ...
            temp_Y_entry_list(time_index)=Y_entry_num;
            temp_Z_entry_list(time_index)=Z_entry_num;
    ...
    Y_entry(:,repeat_i)=temp_Y_entry_list;
    Z_entry(:,repeat_i)=temp_Z_entry_list;
end
...
t=T/1000;
k=linear_threshold(K,HP)/1000;

if T==0
    propensity=0;
else
    propensity=T*t^n/(t^n+k^n);
    %     propensity
end
...
function thr=linear_threshold(K,HP)    

thr=max(0.1*K, K-4*HP);
% thr=K;
end