function X_list=Gillespie_switch_actinorm_v2(lambda_p, lambda_n, lambda_d, phos_deg_ratio, delay_mean, delay_cv, activator,repeat_time)
...
L_A=length(activator);
activator0=[activator,activator(1)];


switch_scale=400*1.25;
%% Simulation details
% repeat_time=1;
t=0;
max_t=144000;
...
%         Kd=10.;
        ...
        t_tilda=floor(t);
        A=(1+t_tilda-t)*activator0(1+rem(t_tilda,L_A))+(-t_tilda+t)*activator0(2+rem(t_tilda,L_A));
        
        fraction_freeA=max(1-x(4)/A,0);
        prop_vector(1) = lambda_p*fraction_freeA; % Protien production
...
% thr=K-5*HP;
% thr=K*(1+(HP*(HP+300)*(HP-1000))/620^3);
% thr=K-0.9*K*(HP>=100);
